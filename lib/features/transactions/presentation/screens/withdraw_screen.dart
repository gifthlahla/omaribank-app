...
class _WithdrawScreenState extends ConsumerState<WithdrawScreen> {
  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      final confirm = await ConfirmationDialog.show(
        context,
        title: AppStrings.withdrawTitle,
        content: 'Are you sure you want to withdraw \$${amount.toStringAsFixed(2)}?',
      );

      if (confirm == true) {
        await ref.read(walletControllerProvider.notifier).withdraw(amount);
        ...
        if (state.hasError) {
          ...
          context.showSnackBar('Withdrawal successful!');
          context.pop();
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text(AppStrings.withdrawTitle)),
      body: SafeArea(
        child: Padding(
